...
(* One tick of the core logic. *)
Definition encodeLoopEnable
  {A B : SignalType}
  (f : seqType A * seqType B -> ident (seqType B))
  (en_i_state : (seqType (Pair Bit A) * seqType B)) :
  ident (seqType B) :=
    let state := snd en_i_state in
    let en_i := fst en_i_state in
    let (en, i) := split en_i in
...
Instance CircuitSemantics : Cava seqType :=
  { cava := ident;
    ...
    loopDelaySEnableR _ _ resetValue en f input :=
         loopSeqS resetValue (encodeLoopEnable f) (mkpair en input)
   }.
